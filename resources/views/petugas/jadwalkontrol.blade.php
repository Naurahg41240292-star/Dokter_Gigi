...
<body class="flex">
    ...
    <tr class="border-b border-gray-50 table-row-hover transition">
        ...
        <td class="py-4 text-center">
            <!-- Tombol Aksi: Cek apakah sudah ada Rekam Medis -->
            @if($item->rekamMedis)
                <a href="{{ route('petugas.edit-rm', $item->rekamMedis->id) }}" class="inline-flex items-center gap-2 bg-primary-600 text-white px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-primary-700 transition shadow-sm">
                    <i class="fas fa-file-medical"></i> Isi Rekam Medis
                </a>
            @else
                <span class="text-slate-400 text-xs">Belum ada RM</span>
            @endif
        </td>
    </tr>
    @empty
    ...
    <tr class="border-b border-gray-50 table-row-hover transition">
        ...
        <td class="py-4">
            ...
            @elseif($item->status == 'Sedang Berjalan')
                <span class="bg-blue-100 text-blue-700 px-2 py-1 rounded-md text-xs font-bold">Sedang Berjalan</span>
            @else
                <span class="bg-yellow-100 text-yellow-700 px-2 py-1 rounded-md text-xs font-bold">{{ $item->status }}</span>
            @endif
        </td>
        <td class="py-4 text-center">
            @if($item->rekamMedis)
