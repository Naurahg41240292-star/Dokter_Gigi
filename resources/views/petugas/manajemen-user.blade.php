...
<head>
    ...
    <style>
        ...
        .table-row:hover { background-color: #f8fafc; } /* Hover warna abu sangat pudar */
        .table-row.selected { background-color: #eff6ff; } /* Klik jadi biru muda (primary-50) */
    </style>
</head>
<body class="flex">
        ...
        <header class="-mx-6 lg:-mx-8 -mt-6 lg:-mt-8 px-6 lg:px-8 pt-6 pb-5 mb-8 bg-white border-b border-slate-100 flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-4 w-full md:w-auto justify-end">
                <div class="hidden md:flex items-center bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 w-full md:w-60">
                    ...
                    <input type="text" placeholder="Cari user..." class="ml-2 text-sm outline-none w-full bg-transparent text-slate-600">
                </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left">
                    ...
                    <tbody class="text-sm">
                        ...
                        <!-- Tambahkan class "table-row" untuk efek klik -->
                        <tr class="border-b border-gray-50 table-row transition">
                            ...
                            <td class="py-4">
                                @if($user->role === \App\Enums\Role::DOKTER)
                                    <!-- Ganti warna badge dokter jadi hijau tua (emerald) biar ga biru -->
                                    <span class="bg-emerald-50 text-emerald-700 px-2 py-1 rounded-md text-xs font-bold uppercase">Dokter</span>
                                @else
                                    <!-- Ganti warna badge petugas jadi oranye (amber) biar ga ungu/biru -->
                                    <span class="bg-amber-50 text-amber-700 px-2 py-1 rounded-md text-xs font-bold uppercase">Petugas</span>
                                @endif
                            </td>
    ...
    <!-- ✅ JAVASCRIPT EFEK KLIK BARIS BIRU -->
    <script>
        document.querySelectorAll('.table-row').forEach(row => {
            row.addEventListener('click', function(e) {
                // Abaikan klik jika yang diklik adalah tombol di dalam form (Supaya tombol tetap berfungsi)
                if (e.target.closest('button') || e.target.closest('a') || e.target.closest('form')) {
                    return;
                }

                // Hapus class 'selected' dari semua baris lain
                document.querySelectorAll('.table-row.selected').forEach(r => r.classList.remove('selected'));
                
                // Tambah class 'selected' ke baris yang diklik
