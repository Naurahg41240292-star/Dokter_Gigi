...
<head>
    ...
    <title>Tambah Rekam Medis - D'Smile Dental Clinic</title>
...
<body class="flex">
    ...
    <main class="flex-1 ml-[260px] p-6 lg:p-8">
        ...
        <header class="-mx-6 lg:-mx-8 -mt-6 lg:-mt-8 px-6 lg:px-8 pt-6 pb-5 mb-8 bg-white border-b border-slate-100 flex justify-between items-center">
            <div>
                <h2 class="text-xl font-bold text-slate-800">Tambah Rekam Medis</h2>
                <p class="text-sm text-slate-500">Input catatan medis pasien baru.</p>
            </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 md:p-8 max-w-5xl mx-auto">
            <form action="{{ route('petugas.simpan-rm') }}" method="POST">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        <select name="pasien_id" required class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 bg-white">
                            ...
                            @foreach ($pasiens as $pasien)
                                <option value="{{ $pasien->id }}">{{ $pasien->nama }} (NIK: {{ $pasien->nik }})</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-2">Dokter Penanggung Jawab <span class="text-red-500">*</span></label>
                        <input type="text" name="dokter" required class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500" placeholder="Nama Dokter">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-2">Tanggal Kunjungan <span class="text-red-500">*</span></label>
                        <input type="date" name="tanggal_kunjungan" required class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500">
                    </div>
                    <div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="keluhan" rows="3" class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 resize-none" placeholder="Keluhan utama pasien..."></textarea>
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-2">Diagnosa</label>
                        <input type="text" name="diagnosa" class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500" placeholder="Hasil diagnosa">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-2">Tindakan / Perawatan</label>
                        <input type="text" name="tindakan" class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500" placeholder="Tindakan yang dilakukan">
                    </div>
                    <div class="md:col-span-2">
                        <label class="block text-sm font-semibold text-slate-700 mb-2">Resep Obat</label>
                        <input type="text" name="resep_obat" class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500" placeholder="Obat yang diresepkan">
                    </div>
                </div>
                ...
                <div class="flex justify-end gap-4 pt-6 border-t border-slate-100">
                    ...
                    <button type="submit" class="px-6 py-2.5 bg-primary-600 text-white rounded-xl text-sm font-bold hover:bg-primary-700 shadow-md shadow-blue-500/20 transition flex items-center gap-2">
                        <i class="fas fa-save text-xs"></i> Simpan Rekam Medis
                    </button>
                </div>
            </form>
        </div>
